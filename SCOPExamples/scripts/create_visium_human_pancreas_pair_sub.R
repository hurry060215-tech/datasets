...
create_visium_human_pancreas_pair_sub <- function(
  ...
  if (isTRUE(run_integration)) {
    ...
    # Keep only ordinary serializable payloads in the distributed fixture;
    # the PRECASTObj is optional-backend state and is not needed by the plots.
    integration <- pair@tools[["SpatialIntegration"]]
    if (is.list(integration$methods)) {
      for (method_name in names(integration$methods)) {
        integration$methods[[method_name]]$raw_result <- NULL
      }
      pair@tools[["SpatialIntegration"]] <- integration
    }
  }
